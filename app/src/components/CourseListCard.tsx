  ...
  return (
    <div className="overflow-hidden shadow-lg rounded-lg h-90 w-full cursor-pointer m-auto">
      <Link to={`/courses/edit/${course.id}`} className="w-full block h-full">
        <img src={course.cover_image} alt="blog photo" className="max-h-40 w-full object-cover" />
        <div className="bg-white w-full p-4">
          ...
          <div className="flex items-center mt-4">
            ...
            <div className="flex flex-col justify-between ml-4 text-sm">
              <p className="text-gray-800">{course.creator.name}</p>
              <p className="text-gray-400">{course.created_at} - 6000 min read</p>
            </div>
          </div>
        </div>
      </Link>
